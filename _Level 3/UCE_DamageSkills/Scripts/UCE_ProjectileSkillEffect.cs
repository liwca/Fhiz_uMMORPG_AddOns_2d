...
public class UCE_ProjectileSkillEffect : SkillEffect
{
    ...
    public void Apply()
    {
        ...
        Debug.Log("skill:" + parentSkill.damage);

        if (target is Player && ((Player)caster).UCE_SameCheck((Player)target, parentSkill.affectSelf, parentSkill.affectPlayers, parentSkill.affectOwnParty, parentSkill.affectOwnGuild, parentSkill.affectOwnRealm, parentSkill.reverseTargeting))
        {
            if (target.isAlive)
                targets.Add(target);
        }
        else if (target is Monster && parentSkill.affectEnemies)
        {
            if (target.isAlive)
                targets.Add(target);
        }
